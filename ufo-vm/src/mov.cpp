...
void* _mov(int instr_type, int size, void* entry)
{
	...
	case 3:
		...
		{
		case 0:
			*(uint8_t*)PTR_SUM(data_base, addr) = *(uint8_t*)PTR_SUM(data_base, *(uint16_t*)op_2);
			break;
		case 1:
			*(uint16_t*)PTR_SUM(data_base, addr) = *(uint16_t*)PTR_SUM(data_base, *(uint16_t*)op_2);
			break;
		case 2:
			*(uint32_t*)PTR_SUM(data_base, addr) = *(uint32_t*)PTR_SUM(data_base, *(uint16_t*)op_2);
			break;
		case 3:
			*(uint64_t*)PTR_SUM(data_base, addr) = *(uint64_t*)PTR_SUM(data_base, *(uint16_t*)op_2);
			break;
		}
		return PTR_SUM(entry, 5);
	}
